...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = Mangacore.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new Mangacore.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new Mangacore.HDPublicKey(c.xPubKey);
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var privateKey = new Mangacore.PrivateKey(privateKeyWif);
  var address = privateKey.publicKey.toAddress().toString();
  var addrBuff = new Buffer(address, 'ascii');
  var actualChecksum = Mangacore.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = Mangacore.encoding.Base58Check.decode(encryptedPrivateKeyBase58).toString('hex').substring(6, 14);
...
API._buildSecret = function(walletId, walletPrivKey, network) {
  if (_.isString(walletPrivKey)) {
    walletPrivKey = Mangacore.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new Mangacore.encoding.Base58(widHex).toString();
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var widHex = Mangacore.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = Mangacore.PrivateKey.fromString(secretSplit[1]);
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Mangacore.HDPrivateKey(derivedXPrivKey);
...
API.prototype._addSignaturesToMangacoreTx = function(txp, t, signatures, xpub) {
  if (signatures.length != txp.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');
  var mangacore = Mangacore;


  var i = 0,
    x = new mangacore.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = txp.inputs[i];
    try {
      var signature = mangacore.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(txp.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: mangacore.crypto.Signature.SIGHASH_ALL | mangacore.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      }
      ;
...
API.prototype._applyAllSignatures = function(txp, t) {
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToMangacoreTx(txp, t, x.signatures, x.xpub);
  });
};
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var walletPrivKey = opts.walletPrivKey || new Mangacore.PrivateKey();
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new Mangacore.PrivateKey(walletPrivKey)).toPublicKey().toString(),
    network: network,
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = Mangacore.PrivateKey.fromString(c.walletPrivKey);
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new Mangacore.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new Mangacore.HDPrivateKey(this.credentials.xPrivKey)
