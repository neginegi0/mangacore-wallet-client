...
Credentials.create = function(network) {
  ...
  x.network = network;
  x.xPrivKey = (new Mangacore.HDPrivateKey(network)).toString();
...
Credentials.fromExtendedPublicKey = function(xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  var x = new Credentials();
  x.xPubKey = xPubKey;
  x.entropySource = Mangacore.crypto.Hash.sha256sha256(entropyBuffer).toString('hex');
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
Credentials.prototype._hashFromEntropy = function(prefix, length) {
  ...
  var b2 = Mangacore.crypto.Hash.sha256hmac(b, new Buffer(prefix));
...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    var xPrivKey = new Mangacore.HDPrivateKey.fromString(this.xPrivKey);
  ...
  if (this.entropySourcePath) {
    ...
    this.entropySource = Mangacore.crypto.Hash.sha256sha256(seed).toString('hex');
  }

  if (this.entropySource) {
    // request keys from entropy (hw wallets)
    var seed = this._hashFromEntropy('reqPrivKey', 32);
    var privKey = new Mangacore.PrivateKey(seed.toString('hex'), network);
    ...
    this.entropySource = Mangacore.crypto.Hash.sha256(requestDerivation.privateKey.toBuffer()).toString('hex');
  }
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  var xPrivKey = new Mangacore.HDPrivateKey(this.getKeys(password).xPrivKey, this.network);
  var deriveFn = !!this.compliantDerivation ? _.bind(xPrivKey.deriveChild, xPrivKey) : _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey);
...
Credentials.fromOldCopayWallet = function(w) {
  function walletPrivKeyFromOldCopayWallet(w) {
    ...
    var privKey = new Mangacore.PrivateKey.fromBuffer(Mangacore.crypto.Hash.sha256(seedBuf));
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      ...
      requestDerivation = (new Mangacore.HDPrivateKey(credentials.xPrivKey))
        .deriveChild(path).hdPublicKey;
    } else {
      // this 
      var path = Constants.PATHS.REQUEST_KEY_AUTH;
      requestDerivation = (new Mangacore.HDPublicKey(xPubStr)).deriveChild(path);
    }

    // Grab Copayer Name
    var hd = new Mangacore.HDPublicKey(xPubStr).deriveChild('m/2147483646/0/0');
