...
var PrivateKey = Mangacore.PrivateKey;
var PublicKey = Mangacore.PublicKey;
var crypto = Mangacore.crypto;
var encoding = Mangacore.encoding;
...
/* TODO: It would be nice to be compatible with mangacoind signmessage. How
 * the hash is calculated there? */
Utils.hashMessage = function(text) {
  $.checkArgument(text);
  var buf = new Buffer(text);
  var ret = crypto.Hash.sha256sha256(buf);
  ret = new Mangacore.encoding.BufferReader(ret).readReverse();
  return ret;
};
...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(Mangacore.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = Mangacore.PrivateKey.fromString(privKey);
  return Mangacore.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new mangacore.HDPublicKey(item.xPubKey);
    return xpub.deriveChild(path).publicKey;
  });

  var mangacoreAddress;
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      mangacoreAddress = mangacore.Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      mangacoreAddress = mangacore.Address.fromPublicKey(publicKeys[0], network);
      break;
  }

  return {
    address: mangacoreAddress.toString(),
    path: path,
...
Utils.verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new Mangacore.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
Utils.buildTx = function(txp) {
  ...
  var t = new mangacore.Transaction();
  ...
  if (txp.toAddress && txp.amount && !txp.outputs) {
    ...
    _.each(txp.outputs, function(o) {
      ...
      if (o.script) {
        t.addOutput(new mangacore.Transaction.Output({
          script: o.script,
          satoshis: o.amount
        }));
      } else {
  ...
  // Validate inputs vs outputs independently of Mangacore
