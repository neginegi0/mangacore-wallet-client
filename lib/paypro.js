...
PayPro.get = function(opts, cb) {
  ...
  var PP = new MangacorePayPro();

  opts.headers = opts.headers || {
    'Accept': MangacorePayPro.JSON_PAYMENT_REQUEST_CONTENT_TYPE,
  ...
  http(opts, function(err, dataBuffer) {
    ...
    try {
      var body = MangacorePayPro.PaymentRequest.decode(dataBuffer);
    ...
    var decodedDetails = MangacorePayPro.PaymentDetails.decode(serializedDetails);
    var pd = new MangacorePayPro();
    ...
    var addr = new mangacore.Address.fromScript(new mangacore.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount) {
  ...
  var mangacore = Mangacore;
  var output = new MangacorePayPro.Output();
  var addr = new mangacore.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = mangacore.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = mangacore.Script.buildScriptHashOut(addr);
  } else {
...
PayPro.send = function(opts, cb) {
  ...
  opts.method = 'POST';
  opts.headers = opts.headers || {
    'Accept': MangacorePayPro.JSON_PAYMENT_ACK_CONTENT_TYPE,
    'Content-Type': MangacorePayPro.JSON_PAYMENT_CONTENT_TYPE,
    // 'Content-Type': 'application/octet-stream',
  };
  opts.body = payment;
  ...
  http(opts, function(err, rawData) {
    ...
    if (rawData) {
      try {
        var data = MangacorePayPro.PaymentACK.decode(rawData);
        var pp = new MangacorePayPro();
